...
public class ViewPagerIndicator extends View {
    ...
    private void initPaint() {
        ...
        //空心
        paintStrokeLine.setStyle(Paint.Style.STROKE);
        paintStrokeLine.setColor(mDefault_color);
        paintStrokeLine.setAntiAlias(true);
        paintStrokeLine.setStrokeWidth(3);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (GravityType.RIGHT == gravityType) {
            //+ mNum * mHeight - mHeightn
            float n = +mNum * mHeight - mHeight;
//            L.d("轮播点减" + n);
            float wt = ScreenUtil.dip2px(context, (float) (8+mNum * 10));
//            L.d("轮播点x坐标" + wt);
//            L.d("轮播点宽度" + width);
            canvas.translate(width - wt, height / 2);
        } else {
        ...
        switch (mIndicatorType) {
            ...
            case IndicatorType.CIRCLE_LINE://圆线
                ...
                break;
            case IndicatorType.RECT_LINE:
//                L.d("轮播位置--" + mPosition);
                if (mPosition == (mNum - 1)) {
                    //第一个 线 选中 消失
                    float leftClose = -(mNum) * 0.5f * mDistance - mRadius;
                    float rightClose = leftClose + 2 * mRadius + mOffset + (mPosition + 1) * mDistance;
                    float topClose = -mRadius;
                    float bottomClose = mRadius;
                    RectF rectClose = new RectF(leftClose, topClose, rightClose, bottomClose);// 设置个新的长方形
                    canvas.drawRoundRect(rectClose, mRadius, mRadius, paintStroke);
                    ...
                    if (mPosition < mNum - 1) {
                        ...
                        float bottomOpen = mRadius;
                        RectF rectOpen = new RectF(leftOpen, topOpen, rightOpen, bottomOpen);// 设置个新的长方形
                        canvas.drawRoundRect(rectOpen, mRadius, mRadius, paintStrokeLine);
                    }
                    //圆
                    for (int i = mPosition + 3; i <= mNum; i++) {
                        canvas.drawCircle(-(mNum) * 0.5f * mDistance + i * mDistance, 0, mRadius, paintStrokeLine);
                    }
                    for (int i = mPosition - 1; i >= 0; i--) {
                        canvas.drawCircle(-(mNum) * 0.5f * mDistance + i * mDistance, 0, mRadius, paintStrokeLine);
                    }
                    ...
                    canvas.drawRoundRect(rectClose, mRadius, mRadius, paintStroke);
                    //圆
                    for (int i = 0; i < (mNum - 1 - mPosition); i++) {
                        canvas.drawCircle(rightClose + (mDistance) * (i + 1) - mRadius, 0, mRadius, paintStrokeLine);
//                        L.d("添加圆--" + i + "---位置==" + (rightClose + mRadius + mDistance));
                    }
                }
                break;
    ...
    private void setStyleable(Context context, AttributeSet attrs) {
        ...
        gravityType = array.getInteger(R.styleable.ViewPagerIndicator_vpi_gravityType, GravityType.CENTER);
    ...
    public void move(float percent, int position, boolean isLeft) {
        ...
        switch (mIndicatorType) {
            case IndicatorType.CIRCLE_LINE://圆线
            case IndicatorType.RECT_LINE://圆线延长
